...
pdht_t *pdht_create(size_t keysize, size_t elemsize, pdht_mode_t mode){
  ...
  /* Change this to support returning PdhtStatus */
...
int main(int argc, char *argv[]){
  int key, val;
  pdht_t *dht = pdht_create(sizeof(int), sizeof(int), PdhtModeStrict);
  key = 1;
  val = 2;
  
  if(c->rank == 0){
    pdht_put(dht, &key, &val);
  }
  val = 0;
  if(c->rank == 0){
    pdht_get(dht, &key, &val);
    printf("val : %d \n", val);
  }
  return 0;
}
