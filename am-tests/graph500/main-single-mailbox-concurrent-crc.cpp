...
void graph_cb(void *elem, void *args, int elem_index, void *ctx) {
    size_t i;
    int *inc_val = (int *)ctx;
    char *my_arg = (char *)args;
    int old_index = __sync_fetch_and_add(inc_val, 1);
    char *my_buf = (char *) elem + old_index;
    *my_buf = my_arg[elem_index];
}
...
int main(int argc, char **argv) {
    ...
    shmem_am_handle_t cb_handle = shmem_insert_cb(SHMEM_AM_PUT, graph_cb, recv_buf_index);
    shmem_barrier_all();
#ifdef VERBOSE
    fprintf(stderr, "PE %d shmem_malloc-ing %llu bytes\n", shmem_my_pe(),
