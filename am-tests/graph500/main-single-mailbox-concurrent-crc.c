...
int levels[16];
int sums[16];
int pWrkx[SHMEM_REDUCE_MIN_WRKDATA_SIZE];
long pSyncx[SHMEM_SYNC_SIZE];
int iterx;
...
#define SEND_HEADER_SIZE 0
#define SEND_BUFFER_SIZE (COALESCING * sizeof(packed_edge))
...
#define SEND_PACKET(my_target_pe) { \
    PREPARE_PACKET(my_target_pe) \
    shmem_put_am(recv_buf, send_bufs_size[my_target_pe], \
                 sizeof(char), my_target_pe, cb_handle, \
                 end_bufs[my_target_pe]->buf, \
...
/* we used fetch and add to find the address, so this idea is now 
 * dead. We will maintain our own index locally now. 
 * we can maybe make this the context or if its global it doesn't matter
 * we might redo the api for this. */
void graph_cb(void *elem, void *args, size_t size, void *ctx) {
    length_in_bytes += sizeof(char);
    cb_ctx_t *inc_val = (cb_ctx_t *)ctx;
...
static inline unsigned check_for_receives(unsigned char **iter_buf,
    ...
    unsigned char *local_iter_buf = *iter_buf;// + sizeof(size_t);
...
int main(int argc, char **argv) {
    ...
    for (i = 0; i < npes; i++) {
        remote_offsets[i] = shmem_longlong_atomic_fetch_add((long long int *)&n_local_edges,
    ...
    cb_ctx_t recv_ctx; //= (cb_ctx_t *)malloc(sizeof(cb_ctx_t));
    int *recv_buf_index = (int *)malloc(sizeof(int));
    recv_ctx.next_offset = recv_buf_index;
    int *inactive_recv_buf_index = (int *)malloc(sizeof(int));
    assert(recv_buf_index && inactive_recv_buf_index);


    shmem_am_handle_t cb_handle = shmem_insert_cb(SHMEM_AM_PUT, graph_cb, &recv_ctx);
    shmem_barrier_all();
#ifdef VERBOSE
    fprintf(stderr, "PE %d shmem_malloc-ing %llu bytes\n", shmem_my_pe(),
    ...
    const unsigned num_bfs_roots = 100;
    ...
    for (run = 0; run < num_bfs_roots; run++) {
        ...
        *inactive_recv_buf_index = 0;
        ...
        memset(levels, 0x0, nlevels * sizeof(int));
        memset(sums, 0x0, nlevels * sizeof(int));
        iterx = 0;
        ...
        while (1) {
            ...
            *nmessages_global = 0;
            ...
            //printf("%d zeroing : a %d i %d \n", pe, *recv_buf_index, *inactive_recv_buf_index);
            *recv_buf_index = 0;
...
//                     pe, iter, duplicates_in_same_wavefront);
        ...
        shmem_int_sum_to_all(sums, levels, nlevels, 0, 0, npes, pWrkx, pSyncx);
